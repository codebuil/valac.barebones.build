...
class BareboneBuilder:
    ...
    def build_kernel(self):
        ...
        self.execute_command('rm  "$1".o'.replace("$1",filename),False)
        self.execute_command("mv -f kernel.bin /tmp/null",False)
        self.execute_command("as -o /tmp/boot.o ./file/boot.s",True)
        fff=f'valac --includedir=./file -c  "$1" --output=/tmp/kernel.o'.replace("$1",filename)
        self.execute_command('mv -f "$1".o /tmp/kernel.o'.replace("$1",filename),False)
    ...
    def copy_file(self):
        ...
        if filename:
            shutil.copy( f"./file/new",filename+".vala")
